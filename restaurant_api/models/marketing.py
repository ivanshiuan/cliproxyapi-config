...
from .base import Base, TimestampedMixin, TenantScopedMixin, uuid7
...
class AssetPlatform(str, enum.Enum):
    LINE = "line"
    INSTAGRAM = "instagram"
    FACEBOOK = "facebook"
...
class AiCampaignStatus(str, enum.Enum):
    PENDING = "pending"
    RUNNING = "running"
    COMPLETED = "completed"
    FAILED = "failed"
...
class MarketingMemory(Base, TenantScopedMixin, TimestampedMixin):
    ...
    memory_type: Mapped[MemoryType] = mapped_column(
        Enum(MemoryType, name="marketing_memory_type"),
        nullable=False,
        index=True,
    )
...
class ContentAsset(Base, TenantScopedMixin, TimestampedMixin):
    ...
    asset_type: Mapped[AssetType] = mapped_column(
        Enum(AssetType, name="content_asset_type"),
        nullable=False,
        index=True,
    )
    platform: Mapped[AssetPlatform] = mapped_column(
        Enum(AssetPlatform, name="content_asset_platform"),
        nullable=False,
    )
    status: Mapped[AssetStatus] = mapped_column(
        Enum(AssetStatus, name="content_asset_status"),
        nullable=False,
...
class AiCampaign(Base, TenantScopedMixin, TimestampedMixin):
    ...
    status: Mapped[AiCampaignStatus] = mapped_column(
        Enum(AiCampaignStatus, name="ai_campaign_status"),
        nullable=False,
    ...
    target_platform: Mapped[AssetPlatform] = mapped_column(
        Enum(AssetPlatform, name="ai_campaign_platform"),
        nullable=False,
    )
