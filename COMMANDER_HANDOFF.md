...
### 0. 🆕🆕 追出一個更深的 bug：圖文選單/歡迎訊息之前也沒真的免打字——已修好，不用你做事

再往下查發現：不只門口 QR 沒加好友，連「已經是好友、點圖文選單」這條路也有洞——圖文選單按鈕跟自動歡迎訊息的按鈕，網址都**沒帶 `?liff=<id>`**，導致就算客人已加好友，點按鈕還是會被前端判定成訪客、跳去打字輸入，不是真的免打字 LINE 身份。這是最初「免打字」需求本身沒做對，不是新回歸。

**已經修好，這部分不用你做任何事**：`/admin/line/richmenu` 跟 `/admin/line/finalize` 現在會自動查已註冊的 LIFF app、把 `?liff=<id>` 接進圖文選單按鈕；歡迎訊息也改成每次加好友都即時組裝、動態帶入目前的 LIFF id（不再是寫死的靜態檔案）。兩邊都有「查不到就優雅退回訪客模式」的容錯，不會因為這步失敗就整個壞掉。已加 11 個新測試涵蓋（含「查到已存在 LIFF app」「查詢失敗容錯」「舊值不會蓋掉新值」）。

### 0-A. 門口海報 QR 現在只有「訪客玩」——要「掃了先加好友」得補一個值

檢查後發現：門口海報的 QR 目前直接連到輪盤頁本身（一般相機掃描，不經過 LINE），客人可以立刻玩，但**不會**變成你的 LINE 好友、也不是真的會員身份——只有已經是好友的人點圖文選單才會走到「真身份、免打字」的路（上面 0. 那個 bug 修好後，這條路現在才是真的走得通）。這跟最一開始「掃碼→加好友→玩→綁會員」的目標還差一步。
