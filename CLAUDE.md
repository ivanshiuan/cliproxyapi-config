...
11. **交付前一定要深度複盤，不是寫完就丟出去** — 測試綠燈只代表「測到的行為符合預期」，不代表「這個功能真的端到端能用」。每個功能寫完後，自己重新走一次完整使用者路徑（不只是各別函式），去追：這個值從哪裡來？下一步真的有人接住它嗎？有沒有哪個環節看起來合理但其實沒真的接上？寫完先當自己的對抗者去找漏洞，找不到才交付；不是複盤一次就算了，覺得有一點不踏實就再查一輪，查到有把握為止。這條規則存在的理由：這個專案已經發生過「圖文選單/歡迎訊息按鈕都測試通過、程式碼也在跑，但沒人發現網址少了 `?liff=` 參數，導致『免打字』核心需求整個沒生效」——bug 是在使用者已經照著 SOP 操作到一半才被揪出來的，不是在複盤時找到的。晚發現的 bug 永遠比早發現貴：要改的地方更多、要重新驗證的路徑更長、使用者已經照錯的步驟做了要重來。寫完不檢查等於考卷寫完不檢查就交出去。
