...
### 行為準則（Karpathy 四原則精華，取自 andrej-karpathy-skills）
- **先想再寫**：非顯而易見的假設要先講明；有多種解讀就列出來（照第 10 條用 AskUserQuestion），不要默默選一個；有更簡單的做法就直說。
- **最簡優先**：只寫解決問題的最少程式碼。不加沒被要求的功能、不為單次使用寫抽象層、不加沒人要的「彈性」、不替不可能的情境寫錯誤處理。200 行能用 50 行寫就重寫。
- **外科手術式修改**：只碰任務需要的行。不順手「改善」旁邊的程式碼/註解/格式、不重構沒壞的東西；自己改動造成的孤兒 import/變數要清掉，既有 dead code 提出來但不要動。檢驗：**每一行 diff 都能追溯到需求**。
- **目標驅動**：把任務轉成可驗證目標（「修 bug」→「先寫重現測試再讓它過」）；完成的定義永遠是 `make full-check` 全綠，不是「看起來能動」。
