...
SID.meta = {
  ...
  snapshot: "2026-06-19",
  live: false,
  // 傷病/先發未經查證者一律不填（守鐵律）；live refresh 可由 API-Football /injuries 同步
};

/* ---------------------------------------------------------------------
 * RESULTS｜賽後實際結果（回測用）
 * 賽後填入 { gh, ga }（主隊進球, 客隊進球），SID.runBacktest() 會自動
 * 計算 Brier / Log Loss / 命中率 / 可靠度。未填的場次自動略過。
 * 範例："WC2026-H-ESP-KSA": { gh: 3, ga: 0 }
 * ------------------------------------------------------------------- */
SID.results = {
  // 四場標的賽前狀態：尚未開賽，留空待賽後回填
};
