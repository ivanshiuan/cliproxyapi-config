...
db-truncate: ## Wipe all rows (DB owner only; bypasses ledger append-only rules)
	PGPASSWORD=$$RESTO_DB_PASSWORD psql -h $$RESTO_DB_HOST -U $$RESTO_DB_USER -d $$RESTO_DB_NAME -c \
		"TRUNCATE TABLE order_events, order_payments, order_discounts, order_lines, orders, table_sessions, dining_tables, waste_events, staff_meal_events, tasting_events, time_clocks, shifts, leave_requests, customer_points_ledger, customers, stock_movements, recipes, menu_items, menu_categories, ingredients, cash_drawer_sessions, audit_log, employees, stores, tenants CASCADE;"
